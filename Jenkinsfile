...
//		fda-git-auth-id		        - Git login as Jenkins "Username with Password" credential
//      fda-ssc-ci-token-id         - Fortify Software Security Center "CIToken" authentication token as Jenkins Secret credential
//      fda-edast-auth-id           - Fortify Scan Central DAST authentication as "Jenkins Username with Password" credential
//      fda-nexus-iq-token-id       - Sonatype Nexus IQ user token as Jenkins Secret credential in form of "user code:pass code"
//      fda-debricked-token-id      - Debricked API access token as Jenkins Secret credential
...
def dockerImage
def dockerContainer
def dockerContainerName = "fda-jenkins"
def dastScanName = "fda-jenkins"
...
pipeline {
    ...
    environment {
        ...
        APP_NAME = "FortifyDemoApp"         		        // Application name
        APP_VER = "1.0-build"                               // Application version
        COMPONENT_NAME = "fda"                              // Short form component name
        DOCKER_IMAGE_NAME = "fortifydemoapp"             // Docker image name
        GIT_URL = scm.getUserRemoteConfigs()[0].getUrl()    // Git Repo
        JAVA_VERSION = 11                                   // Java version to compile as

        // Credential references
        GIT_CREDS = credentials('fda-git-auth-id')
        SSC_CI_TOKEN = credentials('fda-ssc-ci-token-id')
        NEXUS_IQ_AUTH_TOKEN = credentials('fda-nexus-iq-token-id')
        DEBRICKED_TOKEN = credentials('fda-debricked-token-id')
        ...
        SSC_APP_NAME = "${params.SSC_APP_NAME ?: 'FortifyDemoApp'}" // Name of Application in SSC to upload results to
        ...
        SSC_SENSOR_VER = "${params.SSC_SENSOR_VER ?: '23.1'}" // ScanCentral SAST Sensor version
        ...
        NEXUS_IQ_APP_ID = "${params.NEXUS_IQ_APP_ID ?: 'FortifyDemoApp'}" // Sonatype Nexus IQ App Id
        DEBRICKED_APP_ID = "${params.DEBRICKED_APP_ID ?: 'fortify-presales/FortifyDemoApp'}" // Debricked App Id
                ...
                script {
                    if (params.SCANCENTRAL_DAST && params.USE_DOCKER) {
                        ...
                        if (params.USE_FCLI) {
                            ...
                            withCredentials([usernamePassword(credentialsId: 'fda-edast-auth-id', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
            ...
            steps {
                script {
                    ...
                    if (params.RELEASE_TO_GITHUB) {
                        docker.withRegistry('https://ghcr.io', 'fda-git-auth-id') {
