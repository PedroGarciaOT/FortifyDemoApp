...
class TestApp:
    ...
    def test_search_for_product(self, setup):
        chrome_driver = setup

        chrome_driver.get(APP_URL)
        chrome_driver.maximize_window()

        title = "Fortify Demo App"
        assert title == chrome_driver.title

        search_input = WebDriverWait(chrome_driver, 10).until(lambda x: x.find_element(By.ID, "keywords-search-input"))
        search_input.click()
